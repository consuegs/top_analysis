...
void run()
{
   ...
   TH2 *cov_total=unfold.GetEmatrixTotal("cov_total",";bin",0,0,false);
   TH2 *cov_sys=unfold.GetEmatrixSysUncorr("cov_sys",";bin",0,0,false);
   TH2 *prob_matrix=unfold.GetProbabilityMatrix("prob_matrix",0,false);
   ...
   std::cout<<hist_unfolded->GetBinContent(1)<<"   "<<hist_unfolded->GetBinError(1)<<std::endl;
   std::cout<<hist_unfolded->GetBinContent(2)<<"   "<<hist_unfolded->GetBinError(2)<<std::endl;
   
   std::cout<<hist_folded->GetBinContent(1)<<"   "<<hist_folded->GetBinError(1)<<std::endl;
   std::cout<<hist_folded->GetBinContent(2)<<"   "<<hist_folded->GetBinError(2)<<std::endl;
