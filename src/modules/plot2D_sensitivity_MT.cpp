...
//Here MT and MET are used as binning variables
...
void run()
{
   io::RootFileSaver saver(TString::Format("binningUnfolding%.1f.root",cfg.processFraction*100),"plot2D_sensitivity_MT");
   ...
   // ~for(std::vector<float> met_bins : {met_bins1}){
      ...
      for(std::vector<float> mt_bins : {mt_bins1,mt_bins2,mt_bins3}){
         ...
         for (TString bkgSample :{"SingleTop","WJetsToLNu","DrellYan","WW","WZ","ZZ","ttZ_SM","ttH_SM"}) {     //Add all SM backgrounds except ttbar
            add_Categories("2d_MetVSMT_nextLep/"+bkgSample,histReader,temp_hist);
            if (bkgSample=="SingleTop") add_Categories("2d_MetVSMT_nextLep/"+bkgSample,histReader,SMbkg);
         ...
         for (TString signalSample :{"T1tttt_1200_800","T1tttt_1500_100","T2tt_650_350","T2tt_850_100","DM_pseudo_50_50","DM_scalar_10_10","DM_scalar_1_200"}){
            ...
            add_Categories("2d_MetVSMT_nextLep/"+signalSample,histReader,signal);
            signal=hist::rebinned(signal,met_bins,mt_bins);
            ...
            for (TString sensitivity_type : {"SplusB","sqrtB"}){
               ...
               can.RedrawAxis();
               TString plotLoc="Binning_Met"+std::to_string(numberBinningSchemeMet)+"_MT"+std::to_string(numberBinningSchemeMT)+"/"+sensitivity_type+"/"+signalSample;
         ...
         int nTotBins=(met_bins.size()-1)*(mt_bins.size()-1);
         ...
         for (TString bkgSample :{"SingleTop","DrellYan","ttZ_SM","ttH_SM","WJetsToLNu","Diboson","TTbar"}) {
            ...
            if (bkgSample=="Diboson"){    //If diboson add samples
               ...
               for (TString dibosonSample :{"WW","WZ","ZZ"}) {
                  if (dibosonSample=="WW") {
                     add_Categories("2d_MetVSMT_nextLep/"+dibosonSample,histReader,temp);
                  }
                  else {
                     add_Categories("2d_MetVSMT_nextLep/"+dibosonSample,histReader,temp2);
                     temp.Add(&temp2);
                  }
               }
            }
            else add_Categories("2d_MetVSMT_nextLep/"+bkgSample,histReader,temp);
            
            temp=hist::rebinned(temp,met_bins,mt_bins);    //Rebin following the current binning scheme
         ...
         TCanvas can;
         TCanvas can_noTTbar;
         can.Divide(met_bins.size()-1,mt_bins.size()-1);
         can_noTTbar.Divide(met_bins.size()-1,mt_bins.size()-1);
         ...
         // Save pie charts
         TString plotLoc="../output/BKGratios/BinningMet_"+std::to_string(numberBinningSchemeMet)+"_MT"+std::to_string(numberBinningSchemeMT)+".pdf";
         can.SaveAs(plotLoc);    //Save as pdf right away since there is not proper vizualisation in TBrwoser for PieCharts
         plotLoc="../output/BKGratios/BinningMet_"+std::to_string(numberBinningSchemeMet)+"_MT"+std::to_string(numberBinningSchemeMT)+"_noTTbar.pdf";
