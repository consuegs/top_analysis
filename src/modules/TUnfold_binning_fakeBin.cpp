...
void run()
{
   ...
   io::RootFileSaver saver(TString::Format(!withScaleFactor ? "TUnfold_fakeBin_%.1f.root" : "TUnfold_fakeBin_SF91_%.1f.root",cfg.processFraction*100),save_path);
   ...
   Double_t total_fakes=0;
   Double_t tau_fakes=0;
   ...
   for(Int_t ievent=0;ievent<dataTree->GetEntriesFast();ievent++) {
      ...
      if (genbinNumber==1) std::cout<<"fail"<<std::endl;
      if (metGen<0 || genDecayMode>3) {
         genbinNumber=fakeBinning->GetStartBin();
         total_fakes+=mcWeight;
         if (genDecayMode>3) tau_fakes+=mcWeight;
      }
   ...
   // ~histDataReco->Scale(1.035e6/histDataReco->Integral());
   // ~histDataTruth->Scale(2.1078e6/histDataTruth->Integral());
   std::cout<<total_fakes<<std::endl;
   std::cout<<tau_fakes<<std::endl;
   ...
   TH1D histRec_fakes=*(histMCGenRec_purity->ProjectionY("Fakes",1,1));
   TH1D histRec_noFakes=*(histMCGenRec_purity->ProjectionY("Fakes",2,-1));
   // ~histRec_fakes.Scale(1/histRec_fakes.Integral());
   // ~histRec_noFakes.Scale(1/histRec_noFakes.Integral());
   saver.save(histRec_fakes,"histRec_fakes");
   saver.save(histRec_noFakes,"histRec_noFakes");
