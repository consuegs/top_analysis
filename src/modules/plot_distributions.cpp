...
void run()
{
   ...
   // ~std::vector<TString> systToPlot = {"Nominal","JESBBEC1_DOWN"};
   // ~std::vector<TString> systToPlot = {"Nominal","JESFlavorRealistic_UP"};
   // ~std::vector<TString> systToPlot = {"Nominal","JESUserDefinedHEM1516_DOWN"};
   // ~std::vector<TString> systToPlot = {"Nominal","TWDS"};
   // ~std::vector<TString> systToPlot = {"Nominal","BSEMILEP_UP"};
   // ~std::vector<TString> systToPlot = {"Nominal","TOP_PT"};
   // ~std::vector<TString> systToPlot = {"Nominal","BTAGL_UNCORR_DOWN"};
   ...
   std::map<const TString,Color_t> colormap = {{"TTbar_diLepton",TColor::GetColor("#bd1f01")},  //CMS enforced colors
                                             {"Diboson",TColor::GetColor("#92dadd")},
                                             {"ttW/Z",TColor::GetColor("#94a4a2")},
                                             {"SingleTop_DS",TColor::GetColor("#832db6")},
   ...
   for (auto const & distr_ : vecDistr){
      plotHistograms(distr_.path,distr_.name,hs,mcSamples_merged,colormap,printNameMap,{systHists_vec},saver,false,false);
      ...
      if (distr_.path.Contains("/emu") || distr_.name == "emu"){  //plot all channels combined
         ...
         if (distr_.name == "emu") {   // fot cutflow plot
            plotHistograms(combinedPath,"all",hs,mcSamples_merged,colormap,printNameMap,{systHists_vec},saver,false,false);
         }
         else {
            plotHistograms(combinedPath,distr_.name,hs,mcSamples_merged,colormap,printNameMap,{systHists_vec},saver,false,false);
         }
      }
   }
   ...
   for (auto const & distr_ : vecDistr2D){
      plotHistograms(distr_.path,distr_.name,hs,mcSamples_merged,colormap,printNameMap,{systHists_vec},saver,false,true,distr_.binEdgesY);
      
      if (distr_.path.Contains("/emu") || distr_.name == "emu"){  //plot all channels combined
         TString combinedPath(distr_.path);
         combinedPath.ReplaceAll("/emu","/all");
         if (distr_.name == "emu") {   // fot cutflow plot
            plotHistograms(combinedPath,"all",hs,mcSamples_merged,colormap,printNameMap,{systHists_vec},saver,false,true,distr_.binEdgesY);
         }
         else {
            plotHistograms(combinedPath,distr_.name,hs,mcSamples_merged,colormap,printNameMap,{systHists_vec},saver,false,true,distr_.binEdgesY);
         }
      }
   }
