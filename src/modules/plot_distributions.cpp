...
void run()
{
   ...
   // ~std::vector<TString> systToPlot = {"applyJerMET"};
   ...
   for (TString syst : systToPlot){
      ...
      systHists* temp = new systHists(syst,TString::Format("multiHists/%s/histograms_merged_%s.root",syst.Data(),cfg.treeVersion.Data()),TString::Format("distributions%.1f",cfg.processFraction*100),(syst=="Nominal" || syst=="jetPileupIDapplied" || syst=="applyJerMET")? samplesToPlot : mcSamples, ttbarSamples, signalSamples, stSamples);
