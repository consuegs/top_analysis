...
hist::Histograms<TH2F> defineHistograms2D(std::vector<TString> const &datasets){
   ...
   for(TString channel:{"/ee","/mumu","/emu","/all"}){
      ...
      hs2D.addHist(channel+"/GenMetDiffMETRel_MET"   ,";p_{T}^{miss};|p_{T}^{miss}-GenMet|/genMET",100,0,500,6000,-5,5);
      hs2D.addHist(channel+"/GenMetDiffMETRel_MET_Puppi"   ,";p_{T}^{miss};|p_{T}^{miss}-GenMet|/genMET",100,0,500,6000,-5,5);
      hs2D.addHist(channel+"/GenMetDiffMETRel_MET_DNN"   ,";p_{T}^{miss};|p_{T}^{miss}-GenMet|/genMET",100,0,500,6000,-5,5);
...
void fillHistograms(systHists_minTree* syst, minTreeReader const &reader, float const &weight){
   ...
   for (const TString& catString : {cat,TString("/all")}){
      ...
      syst->hists2D_.fillweight(catString+"/GenMetDiffMETRel_MET"                      ,reader.MET_xy,(reader.genMet-reader.MET_xy)/reader.genMet,weight);
      syst->hists2D_.fillweight(catString+"/GenMetDiffMETRel_MET_Puppi"                      ,reader.PuppiMET_xy,(reader.genMet-reader.PuppiMET_xy)/reader.genMet,weight);
      syst->hists2D_.fillweight(catString+"/GenMetDiffMETRel_MET_DNN"                      ,reader.DNN_MET_pT,(reader.genMet-reader.DNN_MET_pT)/reader.genMet,weight);
