...
void run(){
    ...
    // ~test_variables["n_Interactions"] = tuple(linspace(0, 80, 41), "n_Interactions");
    ...
    for(it=test_variables.begin(); it!=test_variables.end(); ++it){
        ...
        TFile output(string(cfg.outputDirectory+"/datacards/"+testVarName+"/ttbar_"+testVarName+"_1d.input.root").c_str(), "RECREATE");
        for (auto m : masses) {
            cout << ">> Writing datacard for variable: " << testVarName << "\n";
            cb.cp().mass({m, "*"}).WriteDatacard((cfg.outputDirectory+"/datacards/"+testVarName+"/ttbar_" + testVarName + "_1d.txt").Data(), output);
        }
    }
}
