...
// dPhi Measurement (mainly copied from pTnunu measurement, so comments/variable names could be still from MET or pTnunu)
...
float roundFloat(const float &in, const int &points){
   float rounder = pow(10.,points);
   return round(in*rounder)/rounder;
}
...
int get_next_end(const TH2D* hist2D, const TH2D resolution, std::vector<float> edges_fine,  const TH1D bothSelection, const TH1D genSelection, int bin_start, int bin_end, float p_min, float s_min, float N_min, float eff_min, ofstream &logger, bool is_logging, bool edge_40GeV, bool merge_Overflow){
	...
	for(int bin_i = current_bin_start; bin_i <= hist2D->GetNbinsX(); bin_i++) {
		...
		if (region.at(1) == 3.14 and merge_Overflow) mergeOverflow = true;
...
void run()
{
   ...
   TTreeReaderValue<float> dPhi_Puppi   (reader, "Phi_recPuppi_xy");
   TTreeReaderValue<float> dPhi_DNN   (reader, "DNN_MET_dPhi_nextLep");
   TTreeReaderValue<float> dPhi_NuNu   (reader, "Phi_NuNu");
   TTreeReaderValue<float> MC_weight   (reader, "N");
   TTreeReaderValue<float> SF_weight   (reader, "SF");
   
   //define additional variables
   // ~float step = 0.0314;			//initial bin width
   // ~float step = 0.0157;			//initial bin width
   float step = 0.04;			//initial bin width
   ...
   for (float i = -3.14; i<=3.14; i+=step) edges_fine.push_back(i);
   ...
   hs2D.addHist("Binning_Optimization/InitialBinning"   ,";min[#Delta#phi(%MET,l)];min[#Delta#phi(p_{T}^{#nu#nu},l)];EventsBIN"           , edges, hist::getWidths(edges), edges, hist::getWidths(edges));
   hs2D.addHist("Binning_Optimization/Resolution"   ,";min[#Delta#phi(p_{T}^{#nu#nu},l)];min[#Delta#phi(%MET,l)] - min[#Delta#phi(p_{T}^{#nu#nu},l)];EventsBIN"           , edges, hist::getWidths(edges), edges_fine, hist::getWidths(edges_fine));
      
   //Temporary: 1D histogram
   hist::Histograms<TH1D> hs({treeName});
   hs.addHist("Temp/bothSelection_Rec", ";min[#Delta#phi(%MET,l)];;EventsBIN", edges, hist::getWidths(edges));
   hs.addHist("Temp/bothSelection_Gen", ";min[#Delta#phi(p_{T}^{#nu#nu},l)];;EventsBIN", edges, hist::getWidths(edges));
      
   //Histogram without weights for efficiency
   hist::Histograms<TH1D> hs_NoWeight({treeName});
   hs_NoWeight.addHist("NoWeight/bothSelection_dPhinunu", ";min[#Delta#phi(p_{T}^{#nu#nu},l)];;EventsBIN", edges, hist::getWidths(edges));
   hs_NoWeight.addHist("NoWeight/genSelection_dPhinunu", ";min[#Delta#phi(p_{T}^{#nu#nu},l)];;EventsBIN", edges, hist::getWidths(edges));
   ...
   while (reader.Next()){
      ...
      if (DNN) {
		  if(*dPhi_NuNu>0 and (merge_Overflow or *dPhi_NuNu <= 3.14)) gen_selection=true;
		  if(*dPhi_DNN>0 and (merge_Overflow or *dPhi_DNN <= 3.14)) rec_selection=true;
		  
		  //Fill Histogramms
		  if(gen_selection && rec_selection){
			  hs2D.fill("Binning_Optimization/InitialBinning", *dPhi_DNN, *dPhi_NuNu);
			  hs2D.fill("Binning_Optimization/Resolution", *dPhi_DNN, *dPhi_DNN - *dPhi_NuNu);
			  hs_NoWeight.fill("NoWeight/bothSelection_dPhinunu", *dPhi_NuNu); 
		  } 
		  if(gen_selection){
			  hs_NoWeight.fill("NoWeight/genSelection_dPhinunu", *dPhi_NuNu);
		  }
		  
		  if(*dPhi_DNN>0 and *dPhi_NuNu>0){
			  hs.fill("Temp/bothSelection_Rec", *dPhi_DNN);
			  hs.fill("Temp/bothSelection_Gen", *dPhi_NuNu);
		  }
	  } else {
		  if(*dPhi_NuNu>0 and (merge_Overflow or *dPhi_NuNu <= 3.14)) gen_selection=true;
		  if(*dPhi_Puppi>0 and (merge_Overflow or *dPhi_Puppi <= 3.14)) rec_selection=true;
		  
		  //Fill Histogramms
		  if(gen_selection && rec_selection){
			  hs2D.fill("Binning_Optimization/InitialBinning", *dPhi_Puppi, *dPhi_NuNu);
			  hs2D.fill("Binning_Optimization/Resolution", *dPhi_Puppi, *dPhi_Puppi - *dPhi_NuNu);
			  hs_NoWeight.fill("NoWeight/bothSelection_dPhinunu", *dPhi_NuNu); 
		  } 
		  if(gen_selection){
			  hs_NoWeight.fill("NoWeight/genSelection_dPhinunu", *dPhi_NuNu);
		  ...
		  if(*dPhi_Puppi>0 and *dPhi_NuNu>0){
			  hs.fill("Temp/bothSelection_Rec", *dPhi_Puppi);
			  hs.fill("Temp/bothSelection_Gen", *dPhi_NuNu);
		  }
	  }
   }
   ...
   TString dir = cfg.outputDirectory+"/binningOpt/1D_dPhi/";
   io::ensurePathForFile(dir);
   //Plot histograms
   io::RootFileSaver saver(TString::Format("plots%.1f.root",cfg.processFraction*100),"binningUnfolding1D_Fabian_OA_dPhi");
   TCanvas can;
   //~ can.SetLogz();
   for(TString var : hs2D.getVariableNames()){
      ...
      tempHist->Draw("colz");
      saver.save(can,var);
      if (var == "Binning_Optimization/InitialBinning" and not DNN) can.SaveAs(dir + std::to_string(roundFloat(step,4)) + "_" + "Initial2D.pdf");
      if (var == "Binning_Optimization/InitialBinning" and DNN) can.SaveAs(dir + std::to_string(roundFloat(step,4)) + "_DNN_" + "Initial2D.pdf");
   }
   ...
   fileAppendix += std::to_string(roundFloat(step,4)) + "_";
   ...
   if(is_logging) {
	   ...
	   else log_file << "dPhi_Puppi ist used\n";
	   log_file << "---------------- Parameters ----------------\n";
	   if (forward) log_file << "Operating Mode: Forward\n" ;
	   else log_file << "Operating Mode: Backward\n" ;
	   log_file << "Initial Binning: " + std::to_string(step) + " steps \n";
   ...
   if (forward) {
	   ...
	   edges_optimized.push_back(3.14);
	...
	TH1D rebinned_bothSelection_eff = hist::rebinned_double( *(hs_NoWeight.getHistogram("NoWeight/bothSelection_dPhinunu", {treeName})), edges_optimized, hist::getWidths(edges_optimized), merge_Overflow, false);
	TH1D rebinned_genSelection_eff = hist::rebinned_double( *(hs_NoWeight.getHistogram("NoWeight/genSelection_dPhinunu", {treeName})), edges_optimized, hist::getWidths(edges_optimized), merge_Overflow, false);
	...
	while(not(p >= p_min and s >= s_min and N_rec >= N_min and eff >= eff_min and res_is_ok)){
		if (forward) {
			...
			if(is_logging) log_file << "Merging it with bin " + std::to_string(round(edges_optimized.at(edges_optimized.size()-3))) + "..." + std::to_string(round(edges_optimized.at(edges_optimized.size()-2))) + "\n";
			...
			if(is_logging) log_file << "Merging it with bin " + std::to_string(round(edges_optimized.at(1))) + "..." + std::to_string(round(edges_optimized.at(2))) + "\n";
			edges_optimized.erase(edges_optimized.begin() + 1);
			for (int i = 0; i < edges_optimized.size(); i++)  log_file << std::to_string(round(edges_optimized.at(i))) + " ";
		}
		...
		rebinned_bothSelection_eff = hist::rebinned_double( *(hs_NoWeight.getHistogram("NoWeight/bothSelection_dPhinunu", {treeName})), edges_optimized, hist::getWidths(edges_optimized), merge_Overflow, false);
		rebinned_genSelection_eff = hist::rebinned_double( *(hs_NoWeight.getHistogram("NoWeight/genSelection_dPhinunu", {treeName})), edges_optimized, hist::getWidths(edges_optimized), merge_Overflow, false);
	...
	if(is_logging) {
		...
		log_file << "Final bin edges: ";
		for (int i = 0; i < edges_optimized.size(); i++)  log_file << std::to_string(edges_optimized.at(i)) + " ";
		log_file << "\n";
		log_file << "\n";
		...
		for (int i = 0; i < edges_optimized.size() - 1; i++) log_file << std::to_string(roundFloat(edges_optimized.at(i),4)) + ", ";
		log_file << std::to_string(roundFloat(edges_optimized.at(edges_optimized.size() - 1),4)) + "}; \n";
		log_file << "END";
		log_file.close();
	}
	...
	std::cout<<"Overflow Events (N_rec): " + std::to_string(hs.getHistogram("Temp/bothSelection_Rec", {treeName})->GetBinContent(hs.getHistogram("Temp/bothSelection_Rec", {treeName})->GetNbinsX() + 1))<<std::endl;
	std::cout<<"Overflow Events (N_gen): " + std::to_string(hs.getHistogram("Temp/bothSelection_Gen", {treeName})->GetBinContent(hs.getHistogram("Temp/bothSelection_Gen", {treeName})->GetNbinsX() + 1))<<std::endl;
